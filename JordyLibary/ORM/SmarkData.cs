...
namespace JordyLibary.ORM
{
    ...
    /// Sqlite数据库
    /// </summary>
    public class Sqlite:IDriver
    {

        #region IDriver 成员
        public IDbConnection Connection
        {
            get { return new SQLiteConnection(); }
        }
        ...
        public IDataParameter CreateParameter(string name, object value, ParameterDirection direction)
        {
            SQLiteParameter sp = new SQLiteParameter("@" + name, value);
            if (value == null)
                sp.Value = DBNull.Value;
            sp.Direction = direction;
            return sp;
    ...
    public class SQL
    {
        ...
        /// <returns>int</returns>
        public int Execute(string connName)
        {
        ...
        /// <param name="type">数据库配置项</param>
        /// <returns>返回值</returns>
        public T GetValue<T>(string connName)
        {
            using (IConnectinContext cc = DBContext.GetConnection(connName))
            {
                return GetValue<T>(cc);
        ...
        /// <param name="type">数据库配置项</param>
        /// <returns>返回对象</returns>
        public T ListFirst<T>(string connName) where T : new()
        {
            using (IConnectinContext cc = DBContext.GetConnection(connName))
            {
                return ListFirst<T>(cc);
        ...
        /// <returns>对象列表</returns>
        public IList<T> List<T>(string connName) where T : new()
        {
            using (IConnectinContext cc = DBContext.GetConnection(connName))
            {
                return List<T>(cc);
        ...
        /// <param name="region">区间</param>
        /// <returns>对象列表</returns>
        public IList<T> List<T>(string connName, Region region) where T : new()
        {
            using (IConnectinContext cc = DBContext.GetConnection(connName))
            {
                return List<T>(cc, region);
